...
import ChatAssessment from "@/conversational_questionnaire/components/ChatAssessment";
...
function StepAssessment() {
  ...
  if (!session) {
    return (
      <Shell step={1}>
        {/*
          The one place in this flow where the mascot state means something the
          copy does not repeat, so it gets a localised label. The announcement
          lives on the text, not on the mascot: an aria-live region wrapped
          around an animated SVG re-announces itself every time the animation
          touches the DOM.
        */}
          ...
          {step.kind === "interest" ? (
            ...
            >
              <LikertScale
                ...
                // Only the interest scale is a feelings question. The context
                // questions ask about facts, where a face would imply the
                // answer has a good and a bad end.
                showMascot
              />
            </QuestionCard>
          ) : null}
 ...
 *
 * Two presentations of the same instrument. Both write the same `GuestSession`,
 * so `/mission`, `/routes`, `/compare` and `/plan` need no knowledge of which
 * one ran, and a learner can switch between deployments without losing answers.
 *
 * The flag is read at build time, so only one flow ships to a given deployment.
 * Clearing it is the rollback — no restore, no code change.
 */
