...
import { clearTranscript, TRANSCRIPT_KEY } from "@/conversational_questionnaire/lib/transcript-storage";
  ...
  return (
    <Shell>
      ...
      <Card className="mt-6">
        ...
        {/* Both keys are named. Telling a learner about one store while a
            second holds the same answers would make this page inaccurate. */}
        <p className="mt-3 text-xs text-muted">
          {storedBefore}
          <code className="text-ink">{SESSION_KEY}</code>
          {" · "}
          <code className="text-ink">{TRANSCRIPT_KEY}</code>
          {storedAfter}
        </p>
      </Card>
      ...
      <Card className="mt-4 border-coral/30">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <Button
            onClick={() => {
              clearSession();
              // The conversational flow keeps its transcript under a second key.
              // Clearing only the session would leave the learner's answers
              // still readable in storage after they asked for deletion.
              clearTranscript();
              setCleared(true);
            }}
