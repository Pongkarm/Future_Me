...
import { formDefinition } from "@/conversational_questionnaire/lib/forms";
import { loadFormId } from "@/conversational_questionnaire/lib/form-storage";
...
export default function RoutesPage() {
  ...
  const result = useMemo<Recommendation | null>(() => {
    ...
    try {
      return recommend(session.interview, session.mission, undefined, {
        itemsPresented: formDefinition(loadFormId()).interestCount,
      });
    } catch (e) {
  ...
  return (
    <Shell step={3}>
      ...
      {/*
        Decorative. The heading already says how many routes were found, so the
        mascot has nothing to add for a screen reader — it points at the list to
        carry the eye there, and takes no label.
      */}
