 ...
 *
 * `showMascot` adds the FutureMe face above each point. It is additive by
 * design: the face sits alongside the growing dot rather than replacing it, and
 * it is hidden below the `sm` breakpoint. On mobile the row is only 54px tall,
 * which would put the face at around 40px — well under the 96px it needs to be
 * readable, so it would add colour and no meaning. The dot and the label carry
 * the value at every width.
 */
  ...
  return (
    ...
    >
      {points.map((point, i) => {
        ...
        const emotion = showMascot ? emotionForValue(point.value) : undefined;
        return (
          <button
            key={point.value}
            ref={(el) => {
          ...
          >
            {emotion ? (
              /*
               * Decorative. The number, the dot and the label already carry the
               * value, so this must never be announced — alt="" and aria-hidden
               * together, because a screen reader that ignores one still has
               * the other. A plain <img> rather than next/image: the file is a
               * static SVG, which next/image does not optimise anyway.
               */
              ...
              <img
                ...
                height={78}
                className={[
                  // 96px is the floor the character is designed to read at, and
                  // the column is ~102px wide inside its padding — anything
                  // smaller adds colour without adding meaning.
                  "hidden w-24 shrink-0 transition-all duration-200 sm:block",
