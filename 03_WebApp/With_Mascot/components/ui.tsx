...
/**
 * `mascot` is opt-in and decorative. The tone already carries the meaning
 * through the border colour *and* the copy, so the character is hidden from
 * assistive technology — it makes a warning feel softer, it does not make it
 * legible. Leave it off for notices that appear in long lists, where a repeated
 * face becomes noise.
 */
export function Notice({
  ...
  return (
    ...
    >
      {mascot ? (
        <FutureMeMascot
          emotion={tone === "warning" ? "not-okay" : "neutral"}
          pose={tone === "warning" ? "idle" : "sit"}
          size={72}
          crop="face"
          animated={false}
