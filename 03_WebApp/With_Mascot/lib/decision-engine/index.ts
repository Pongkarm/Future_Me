...
export function recommend(
  ...
  now: Date = new Date(),
  /**
   * How many interest items the learner was actually shown.
   *
   * Defaults to the whole bank, so every existing call behaves exactly as
   * before. A shorter form passes its own length here and is held to the same
   * *proportion* of answers rather than the same count — see `minAnswersFor`.
   */
  options: { itemsPresented?: number } = {},
): Recommendation {
  const { riasec, answered, total } = normaliseInterests(interview);
  const presented = options.itemsPresented ?? total;
  const minAnswers = minAnswersFor(presented);
  ...
  for (const route of routesData.routes as RouteDef[]) {
    ...
    // `presented`, not `total`: this shares the single completeness floor with
    // gate 1. Measuring against the whole bank here would let a short form
    // clear gate 1 and then fail every route on completeness — the same
    // contradictory double floor that was removed from this file once already.
    const strength = evidenceStrength(score, missionCompleted, reasons, answered, presented);
