...
/**
 * The floor for a form of a given length.
 *
 * The *ratio* is the invariant, not the number. A learner who takes a shorter
 * form must answer the same proportion of it — otherwise a short form either
 * can never clear the gate (if the number is kept) or clears it on far less
 * evidence than the full form needs (if the gate is dropped).
 */
export function minAnswersFor(itemsPresented: number): number {
  return Math.ceil(itemsPresented * MIN_INTEREST_RATIO);
}
...
/**
 * How much the strongest and weakest dimension must differ before the profile
 * counts as saying anything.
 *
 * Below this the engine declines to recommend, because picking a leader out of
 * six near-equal numbers is inventing a preference. Exported so the interface
 * can use the same definition of "distinguishable" rather than a second
 * threshold that drifts away from this one.
 */
export const FLAT_PROFILE_SPREAD = 0.15;

/**
 * The n highest dimensions.
 *
 * Note this always returns n of them, whatever the gaps: it is a ranking, not a
 * claim that all n stand out. Anything presenting these to a learner has to
 * check the spread itself — see FLAT_PROFILE_SPREAD.
 */
