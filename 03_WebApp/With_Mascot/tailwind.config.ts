...
const config: Config = {
  ...
  /*
   * Every folder that writes class names must be listed. A folder left out is
   * not an error — its classes are simply purged, so the markup renders with
   * the class attribute present and no rule behind it. That failure is silent
   * and looks like a layout bug rather than a config one.
   */
  content: [
    "./app/**/*.{ts,tsx}",
    "./components/**/*.{ts,tsx}",
    "./conversational_questionnaire/**/*.{ts,tsx}",
  ],
  theme: {
    extend: {
      colors: {
