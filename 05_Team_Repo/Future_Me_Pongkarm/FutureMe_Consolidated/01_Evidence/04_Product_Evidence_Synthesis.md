...
> **สถานะ:** `mixed` — ต้องอ่านสถานะราย claim และยึด `../00_Governance/SOURCE_AUDIT.md` เมื่อขัดกัน

[← Project Overview](../04_Product_and_UX/01_Product_Overview.md) · [Back to README](../README.md) · [Next: User Experience →](../04_Product_and_UX/02_User_Experience.md)
...
|---|---|---|
| People educated above upper-secondary level working outside their field | **56%** | TDRI, 2025; not limited to new graduates |
| People working below their skill or qualification level | **27%** | TDRI, 2025 |
| Highest-unemployment group | Bachelor's degree holders, new graduates | NSO / NESDC |
| Employer experience requirements | A significant barrier in several STEM fields, varying by occupation | TDRI analysis of 756,300 job postings (23 websites, July 2024–June 2025) |
...
| Global employment coverage in ILO study | 56% across 114 sample countries | ILOSTAT |
...
| Removed claim | Why |
|---|---|
| "68.6% overall mismatch" | Unverified; 68.6% in old TDRI paper referred to model accuracy, not overall workforce mismatch |
| "304,378 job postings" | Superseded by 756,300 job postings from 23 websites in full TDRI STEM report |
| "15–20% wage penalty" / "GDP loss 6–8%" | Quarantined; not directly supported by OECD PIAAC primary documents |
| "60%+ / 65% blanket experience requirement" | Quarantined; experience requirement varies sharply by occupation |
| "WEF: 44% of skills will change" | Updated to the current WEF 2025 figure of **39%** |
...
**The finding that mattered most:** the gap is not simply a shortage of options; it is that
students may not see how a study option connects to later choices. The claim that Thailand has
“more than 50 options” is not used as a counted fact because the taxonomy and unit of counting
were never fixed.
...
|---|---|---|
| **NDLP** — National Digital Learning Platform | National learning platform promoted under Anywhere Anytime | Public material reviewed here does not confirm a RIASEC module, dialogue behaviour or technical API |
| **DEEP** — Digital Education Excellence Platform | Earlier Ministry policy described a digital learning and skill-development ecosystem | Current SSO/API availability and integration rights are unverified |

**Strategic position:** complement, not replace, if technical access and partnership become
available. FutureMe must still work independently in the MVP because the current capabilities
of NDLP/DEEP and the right to integrate are not confirmed.
...
> is our responsibility, not the cloud provider's. See [05 · System Architecture](../06_System_Architecture/02_FutureMe_System_Architecture.md).
...
1. **The QLoRA train and test sets are identical** and contain only ten examples each. They
   ...
   before any model evaluation is reported. This is tracked in
   [07 · Roadmap](../04_Product_and_UX/04_Product_Roadmap.md).
2. **Statistical claims required an audit pass.** Unsupported figures were removed (see §1).
   The audit result is present, but this package does not contain the claimed programmatic
   verification suite, so automated enforcement still needs to be implemented.
...
The annotated link index used by this consolidated repository is in
