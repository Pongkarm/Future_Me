...
> **สถานะ:** `design_assumption` — เป็น product specification ไม่ใช่ผล pilot และยังไม่มี product source code ในคลังนี้

[← Back to README](../README.md) · [Next: Research →](../01_Evidence/04_Product_Evidence_Synthesis.md)
...
Per TDRI's 2025 analysis, **56% of Thai people educated above upper-secondary level work outside
their field of study** and **27% work below their skill or qualification level**. The first
figure must not be narrowed to new graduates. Globally, OECD reports field-of-study mismatch
above 35% across participating countries.

The full evidence base, with sources and the claims we deliberately excluded, is in
[02 · Research and Evidence](../01_Evidence/04_Product_Evidence_Synthesis.md).
...
This is hackathon-stage work with a reviewed research base, product specification, mockups,
target architecture and example AI scripts — not a deployed product. The handover package does
not include the FutureMe backend source, frontend source or verification suite, so their
implementation status cannot be confirmed from this repository. Specifically:
...
See [07 · Roadmap](04_Product_Roadmap.md) for how each of these is meant to be closed.
